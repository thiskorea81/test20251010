<template>
  ...
  const studentLabel = computed(() => {
    const s = students.list.find(v => String(v['학번']) === String(props.hakbun))
    return s ? `${s['이름']}(${s['학번']})` : String(props.hakbun)
  })
  ...
  const list = computed(() => counsel.listById(String(props.hakbun)))
  ...
  function save() {
    ...
    if (editingId.value) {
      counsel.update(String(props.hakbun), editingId.value, {
        content, datetime: dtValue, tags: [...tags.value], files: [...files.value]
      })
    } else {
      counsel.add(String(props.hakbun), content, dtValue, [...tags.value], [...files.value])
    }
    resetForm()
  }
  function edit(rec) {
  ...
  function del(id) {
    ...
    counsel.remove(String(props.hakbun), id)
  }
  function clearAllConfirm() {
    if (!confirm('해당 학생의 모든 상담 기록을 삭제할까요?')) return
    counsel.clearAll(String(props.hakbun))
  }
  function resetForm() {
